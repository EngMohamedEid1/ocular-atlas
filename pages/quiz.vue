...
<template>
  <main class="quiz-page" :dir="copy ? 'rtl' : 'ltr'">
    ...
    <section class="quiz-intro"><p>ORAL, PRACTICAL & WRITTEN EXAM BANK</p><h1>{{text('حدد ما تريد أن تُسأل عنه أولاً','Choose what you want to be examined on')}}</h1><span>{{text('اختر المستوى التشريحي بالتتابع: رئيسي ثم فرعي ثم تحت فرعي ثم مستوى أدق عند وجوده. كل معلومة أساسية تُسأل مرة واحدة فقط، مع تنويع بين الأسئلة المقالية والاختيارية والصح أو الخطأ.','Choose the anatomical level step by step: main, substructure, sub-substructure, then microstructure when available. Each core fact is assessed once only, using a mix of essay, multiple-choice, and true/false questions.')}}</span></section>
