...
<template>
  <main class="quiz-page" :dir="copy ? 'rtl' : 'ltr'">
    ...
    <section class="quiz-intro"><p>ORAL, PRACTICAL & WRITTEN EXAM BANK</p><h1>{{text('حدد ما تريد أن تُسأل عنه أولاً','Choose what you want to be examined on')}}</h1><span>{{text('اختر المستوى التشريحي بالتتابع: رئيسي ثم فرعي ثم تحت فرعي ثم مستوى أدق عند وجوده. يجمع البنك بين حقائق مستقلة وأسئلة ربط وتقييم وتطبيق سريري، من دون إعادة صياغة السؤال أو نموذج الإجابة نفسه.','Choose the anatomical level step by step: main, substructure, sub-substructure, then microstructure when available. The bank combines independent facts with linkage, assessment, and clinical-application questions, without repeating a question stem or model answer.')}}</span></section>
