...
<template>
  <main class="quiz-page" :dir="copy ? 'rtl' : 'ltr'">
    ...
    <section class="quiz-intro"><p>ORAL, PRACTICAL & WRITTEN EXAM BANK</p><h1>{{text('حدد ما تريد أن تُسأل عنه أولاً','Choose what you want to be examined on')}}</h1><span>{{text('اختر المستوى التشريحي بالتتابع: رئيسي ثم فرعي ثم تحت فرعي ثم مستوى أدق عند وجوده. تظهر مع كل إجابة مرجعية دولية مفتوحة ومناسبة لمراجعة المجال التشريحي، من دون إعادة صياغة السؤال أو نموذج الإجابة نفسه.','Choose the anatomical level step by step: main, substructure, sub-substructure, then microstructure when available. Each answer includes an open international reference relevant to its anatomical domain, without repeating a question stem or model answer.')}}</span></section>
...
.question-reference{display:flex;flex-direction:column;gap:4px;margin-top:14px;padding:12px;border:1px solid #2f6675;border-radius:8px;background:#0a2534;color:#c4f6ef;text-decoration:none}.question-reference:hover{border-color:#73e2d1;background:#103747}.question-reference small{color:#70ded0;letter-spacing:.1em}.question-reference strong{font-size:.9rem;line-height:1.4}.question-reference span{color:#a9c4d2;font-size:.8rem;line-height:1.4}
